      ...
      <div className="max-w-7xl mx-auto px-4 py-14">
          ...
          {data.subjects.map((s, i) => (
            <Link key={s.id} to={`/subjects/${s.id}?grade_id=${data.grade.id}`} className="group bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all p-7 animate-fade-up" style={{ animationDelay: `${i * 0.06}s` }}>
              <div className="flex items-center gap-4 mb-4">
                ...
                <div>
                  <h3 className="font-extrabold text-slate-900 group-hover:text-teal-700 transition-colors">{s.name}</h3>
        ...
        <div className="mt-14 bg-white rounded-3xl border border-slate-100 shadow-sm p-8 grid md:grid-cols-3 gap-6">
          {[
            ...
            { icon: '📝', title: 'الاختبارات', desc: 'اختبر نفسك وقيّم مستواك', to: '/exams', color: 'bg-teal-100' },
          ].map((c) => (
        ...
        {user && Number(user.grade) === data.grade.id && (
          <Link to="/dashboard" className="mt-8 block text-center bg-gradient-to-l from-teal-600 to-cyan-700 text-white font-extrabold py-4 rounded-2xl shadow-lg shadow-teal-200 hover:-translate-y-0.5 transition-all">
