...
export default function Pricing() {
  ...
  const pay = async (e) => {
    ...
    setError('');
    try {
      const res = await api.post('/subscription/subscribe', { plan: activeOffer ? mode : undefined, subject_ids: selected });
      if (res.payment_url) {
      ...
      <div className="max-w-xl mx-auto px-4 py-20 text-center">
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-10 animate-fade-up">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <button onClick={() => navigate('/dashboard')} className="bg-teal-600 text-white font-extrabold px-8 py-3.5 rounded-2xl hover:bg-teal-700 transition-colors">📊 ابدأ المذاكرة الآن</button>
      ...
      <div className="max-w-7xl mx-auto px-4 py-14">
        ...
        {promos.length > 0 && (
          <div className="flex gap-4 overflow-x-auto pb-2 mb-8 -mx-4 px-4">
            {promos.map((p) => (
              <div key={p.id} className="min-w-[260px] shrink-0 bg-gradient-to-l from-cyan-600 via-teal-700 to-teal-800 text-white rounded-2xl p-5 shadow-lg">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs font-black bg-white/20 border border-white/25 rounded-full px-3 py-1">{p.badge}</span>
                  <span className="text-[10px] font-bold text-teal-200">فترة محدودة</span>
                </div>
                <h3 className="font-extrabold text-lg leading-7 mb-1">{p.title}</h3>
                <p className="text-sm text-teal-200 leading-6">{p.description}</p>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 mb-8">
          {Object.keys(SECTIONS).map((sid) => (
            <button key={sid} onClick={() => setSection(sid)} className={`px-6 py-4 rounded-2xl text-center transition-all border-2 ${section === sid ? 'bg-gradient-to-br from-teal-600 to-cyan-700 text-white border-teal-300 shadow-xl scale-105' : 'bg-white border-slate-200 text-slate-700 hover:border-teal-300'}`}>
              ...
              {mySection === sid && <span className={`inline-block mt-1.5 text-[10px] font-black px-2 py-0.5 rounded-full ${section === sid ? 'bg-white/20' : 'bg-teal-100 text-teal-700'}`}>صفك الدراسي ✓</span>}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-6">
          {OFFER_META.map((om) => {
            ...
            return (
              <button key={om.id} onClick={() => chooseOffer(om.id)} className={`relative text-right rounded-3xl p-7 transition-all ${active ? 'bg-gradient-to-br from-teal-600 to-cyan-700 text-white shadow-2xl ring-2 ring-teal-300 scale-[1.02]' : 'bg-white border border-slate-200 hover:border-teal-300'}`}>
                {om.id === 'triple' && (
                ...
                <p className={`text-sm leading-6 mb-4 min-h-[3rem] ${active ? 'text-teal-100' : 'text-slate-500'}`}>{om.desc}</p>
                <div className="flex items-end gap-3">
                  <span className={`text-5xl font-black ${active ? 'text-white' : 'text-slate-900'}`}>{offer?.price ?? '—'}</span>
                  <div className="mb-1">
                    <span className={`text-sm font-bold ${active ? 'text-teal-200' : 'text-slate-500'}`}>ر.ع</span>
                    <p className={`text-xs ${active ? 'text-teal-200' : 'text-slate-400'}`}>فصل كامل</p>
                  </div>
                  {offer?.original && <span className={`mb-1 text-sm line-through ${active ? 'text-teal-200' : 'text-slate-400'}`}>{offer.original} ر.ع</span>}
                </div>
              </button>
            );
          })}
        ...
        <button onClick={chooseFree} className={`block w-full max-w-5xl mx-auto mb-8 rounded-3xl p-6 transition-all text-right border-2 ${mode === 'free' ? 'border-cyan-500 bg-gradient-to-l from-teal-600 via-cyan-700 to-cyan-700 text-white shadow-xl ring-2 ring-cyan-300' : 'bg-white border-slate-200 hover:border-teal-300 text-slate-800'}`}>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className={`text-sm ${mode === 'free' ? 'text-teal-200' : 'text-slate-500'}`}>سعر المادة الواحدة في {sectionMeta.label}: <b className={mode === 'free' ? 'text-white' : 'text-slate-800'}>{perSubject} ر.ع</b> — اختر أي عدد من المواد</p>
              </div>
            </div>
            <div className={`text-center rounded-2xl px-6 py-3 ${mode === 'free' ? 'bg-white/15 border border-white/25' : 'bg-slate-50 border border-slate-200'}`}>
              <p className={`text-xs ${mode === 'free' ? 'text-teal-100' : 'text-slate-400'}`}>الإجمالي ({selected.length})</p>
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-8 max-w-5xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            ...
            <span className="text-sm font-bold text-teal-600">{activeOffer ? `عرض ${activeOffer.name}` : `${perSubject} ر.ع للمادة`} • {sectionMeta.label}</span>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-8">
            {sectionSubjects.map((s) => {
              ...
              return (
                <button key={s.id} onClick={() => toggleSubject(s.id)} disabled={disabled} className={`p-4 rounded-2xl border-2 flex flex-col items-center gap-1 transition-all ${isOn ? 'border-teal-600 bg-teal-50' : disabled ? 'border-slate-100 opacity-40 cursor-not-allowed' : 'border-slate-100 hover:border-teal-300 hover:bg-teal-50/40'}`}>
                  <span className="text-2xl">{s.icon}</span>
                  <span className={`text-sm font-bold ${isOn ? 'text-teal-800' : 'text-slate-700'}`}>{s.name}</span>
                  <span className={`text-xs font-black ${isOn ? 'text-teal-600' : 'text-slate-400'}`}>{s.price ?? perSubject} ر.ع</span>
                  <span className={`w-6 h-6 mt-1 rounded-full border-2 flex items-center justify-center text-sm ${isOn ? 'bg-teal-600 border-teal-600 text-white' : 'border-slate-300'}`}>{isOn ? '✓' : ''}</span>
                </button>
              );
            })}
          </div>
          ...
          <div className="bg-slate-50 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4 mb-6">
            <div>
              <p className="text-sm text-slate-500">الإجمالي للدفع ({planLabel}) — فصل كامل</p>
          ...
          <button onClick={openPayment} disabled={selected.length === 0} className="w-full bg-gradient-to-l from-teal-600 to-cyan-700 text-white font-extrabold py-4 rounded-2xl hover:-translate-y-0.5 transition-all shadow-lg shadow-teal-200 disabled:opacity-50 disabled:cursor-not-allowed">
            الدفع والتفعيل الآن 💳
          </button>
      ...
      {showPayment && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={() => setShowPayment(false)}>
          <div className="bg-white rounded-3xl max-w-md w-full p-8 animate-fade-up" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-extrabold text-slate-900">💳 تأكيد الاشتراك</h2>
            ...
            <div className="bg-teal-50 rounded-2xl p-5 mb-6">
              <p className="text-sm text-teal-700 font-bold">{planLabel} — {sectionMeta.label}</p>
              <p className="text-sm text-slate-600 mt-1">{subjects.filter((s) => selected.includes(s.id)).map((s) => s.name).join('، ')}</p>
              <p className="text-2xl font-black text-slate-900 mt-2">{total} ر.ع</p>
            </div>
            <form onSubmit={pay} className="space-y-4">
              <p className="text-sm text-slate-600 text-center">سيتم تحويلك إلى بوابة الدفع الآمنة لإتمام عملية الشراء.</p>
              <button type="submit" disabled={processing} className="w-full bg-gradient-to-l from-teal-600 to-cyan-700 text-white font-extrabold py-4 rounded-2xl hover:-translate-y-0.5 transition-all shadow-lg shadow-teal-200 disabled:opacity-50">
                {processing ? 'جارٍ التحويل...' : `إتمام الدفع — ${total} ر.ع`}
              </button>
              <p className="text-center text-xs text-slate-400">في بيئة التطوير، يُفعّل الاشتراك فوراً بدون دفع حقيقي.</p>
            </form>
          </div>
        </div>
      )}
