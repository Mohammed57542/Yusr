...
export default function QuestionBank() {
  ...
  return (
    <div>
      <div className="bg-gradient-to-br from-teal-700 via-cyan-700 to-cyan-800 text-white">
        <div className="max-w-7xl mx-auto px-4 py-16 text-center">
          ...
          <p className="text-teal-200 text-lg max-w-2xl mx-auto">اختر صفّك ومادتك ووحدتك ومستوى الصعوبة، واحصل على أسئلة عشوائية مع تصحيح فوري وشرح الإجابات.</p>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between mb-8">
              {[1, 2, 3].map((s) => (
                <div key={s} className="flex items-center gap-3 flex-1 last:flex-none">
                  <span className={`w-10 h-10 rounded-2xl flex items-center justify-center font-black ${step === s ? 'bg-teal-600 text-white shadow-lg shadow-teal-200' : step > s ? 'bg-green-500 text-white' : 'bg-slate-200 text-slate-500'}`}>
                    {step > s ? '✓' : s}
                  </span>
                  <span className={`font-bold text-sm hidden sm:block ${step === s ? 'text-teal-700' : 'text-slate-400'}`}>
            ...
            {noResults && (
              <div className="mb-5 bg-white rounded-3xl border border-slate-100 shadow-sm p-8">
                <div className="text-center py-8">
                  ...
                  <button onClick={() => setNoResults(false)} className="bg-teal-600 text-white font-bold px-6 py-2.5 rounded-xl text-sm hover:bg-teal-700 transition-colors">تعديل الفلاتر</button>
                </div>
              </div>
            )}
              ...
              {step === 1 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {grades.map((g) => (
                      <button key={g.id} onClick={() => { setGrade(g.id); setStep(2); }} className={`group p-6 rounded-3xl border-2 text-center transition-all ${grade === g.id ? 'border-teal-600 bg-teal-50' : 'border-slate-100 hover:border-teal-300 hover:bg-teal-50/40'}`}>
              ...
              {step === 2 && (
                <div>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => setStep(1)} className="text-teal-600 text-sm font-bold">← تغيير الصف</button>
                  </div>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                    {subjects.map((s) => (
                      <button key={s.id} onClick={() => { setSubject(s.id); setUnit(''); setLesson(''); }} className={`p-5 rounded-2xl border-2 flex items-center gap-3 transition-all ${subject === s.id ? 'border-teal-600 bg-teal-50' : 'border-slate-100 hover:border-teal-300 hover:bg-teal-50/40'}`}>
                  ...
                  {subject && (
                    <>
                      ...
                      <select value={unit} onChange={(e) => { setUnit(e.target.value); setLesson(''); }} className="w-full md:w-1/2 px-4 py-3 rounded-xl border border-slate-200 bg-white font-bold text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                        <option value="">كل الوحدات</option>
                        {units.map((u) => <option key={u.id} value={u.id}>{u.name}</option>)}
                      </select>
                      <label className="block text-sm font-bold text-slate-700 mb-1.5 mt-5">الدرس (اختياري)</label>
                      <select value={lesson} onChange={(e) => setLesson(e.target.value)} className="w-full md:w-1/2 px-4 py-3 rounded-xl border border-slate-200 bg-white font-bold text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
                      ...
                      <div className="mt-8">
                        <button onClick={() => setStep(3)} className="bg-teal-600 text-white font-extrabold px-10 py-4 rounded-2xl hover:bg-teal-700 transition-colors">التالي: اختر الخيارات ←</button>
                      </div>
                    </>
                  )}
                </div>
              ...
              {step === 3 && (
                <div>
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <button onClick={() => setStep(2)} className="text-teal-600 text-sm font-bold">← السابق</button>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {DIFFICULTY.map((d) => (
                          <button key={d.value} onClick={() => setDifficulty(d.value)} className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all ${difficulty === d.value ? 'bg-teal-600 text-white shadow-lg' : `border border-slate-200 ${d.color} hover:border-teal-400`}`}>
                            {d.label}
                          </button>
                        ))}
                      </div>
                  ...
                  <button onClick={start} disabled={loading} className="w-full bg-gradient-to-l from-teal-600 to-cyan-700 text-white font-extrabold py-4 rounded-2xl hover:-translate-y-0.5 transition-all disabled:opacity-50">
          ...
          <div className="animate-fade-up">
            <div className="bg-white rounded-3xl border border-slate-100 shadow-xl p-10 text-center mb-8">
              ...
              <div className="flex justify-center items-center gap-6 my-6">
                <div className="relative w-36 h-36">
                  <svg className="w-36 h-36 -rotate-90" viewBox="0 0 120 120">
                    ...
                    <circle cx="60" cy="60" r="52" fill="none" stroke={result.score / result.total >= 0.6 ? '#0d9488' : '#ef4444'} strokeWidth="12" strokeLinecap="round" strokeDasharray={`${(result.score / result.total) * 326.7} 326.7`} />
                  </svg>
              ...
              <div className="flex flex-wrap justify-center gap-3">
                <button onClick={() => { setSession(null); setResult(null); setAnswers({}); setStep(1); }} className="bg-teal-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-teal-700 transition-colors">تدريب جديد</button>
              ...
              {session.map((q, qi) => (
                <div key={q.id} className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
                  <div className="flex items-center justify-between mb-4">
                    <span className="bg-teal-100 text-teal-700 font-bold text-sm px-4 py-1.5 rounded-full">سؤال {qi + 1}</span>
                  ...
                  <div className="space-y-3">
                    {q.options.map((opt, i) => {
                      ...
                      return (
                        <button key={i} onClick={toggle} className={`w-full flex items-center gap-4 p-4 rounded-2xl border-2 text-right transition-all ${selected ? 'border-teal-600 bg-teal-50' : 'border-slate-100 hover:border-teal-300 hover:bg-teal-50/40'}`}>
                          <span className={`w-9 h-9 rounded-xl flex items-center justify-center font-black shrink-0 ${selected ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-600'}`}>{isMulti ? (selected ? '✓' : letters[i]) : letters[i]}</span>
