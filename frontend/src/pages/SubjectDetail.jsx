    ...
    <div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        {user && (
          <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-6 mb-8">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-3">
              ...
              <span className="text-2xl font-black text-teal-700">{progress.percentage}%</span>
            </div>
            <div className="h-3 rounded-full bg-slate-100 overflow-hidden">
              <div className="h-full rounded-full bg-gradient-to-l from-teal-500 to-cyan-600 transition-all duration-700" style={{ width: `${progress.percentage}%` }} />
            </div>
          </div>
        )}
        ...
        <div id="subject-sections" className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10 scroll-mt-24">
          {SECTION_DEFS.map((s) => (
            <button key={s.id} onClick={() => scrollTo(s.id)} className={`group bg-white rounded-2xl border p-5 text-right transition-all ${tab === s.id ? 'border-teal-400 ring-2 ring-teal-200' : 'border-slate-100 hover:border-teal-200 hover:shadow-md'}`}>
        ...
        <div className="space-y-8">
          {tab === 'lessons' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              <div className="flex items-center justify-between mb-6">
                ...
                <Badge color="bg-teal-100 text-teal-700">{progress.percentage}% إنجاز</Badge>
              </div>
              {lessons.length === 0 ? (
                ...
                <div className="space-y-3">
                  {lessons.map((l, i) => (
                    <Link key={l.id} to={`/lessons/${l.id}`} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:border-teal-200 hover:bg-teal-50/40 transition-all group">
                      <div className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${l.completed ? 'bg-green-100 text-green-600' : l.locked ? 'bg-slate-100 text-slate-500' : 'bg-teal-100 text-teal-700'} group-hover:scale-105 transition-transform`}>
                        {l.completed ? '✓' : l.locked ? '🔒' : `0${i + 1}`}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="font-bold text-slate-800 group-hover:text-teal-700 leading-6">{l.title}</p>
                      ...
                      {l.locked ? (
                        ...
                        <span className="text-teal-600 group-hover:translate-x-[-4px] transition-transform shrink-0">▶</span>
                      )}
                    </Link>
                  ))}
                </div>
          ...
          {tab === 'summaries' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to={`/library?subject=${subject.id}&grade=${gradeId}`} className="text-teal-600 font-bold text-sm">المكتبة الكاملة ←</Link>
              </div>
              {resources.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {resources.map((r) => (
                    <Link key={r.id} to={`/library?file=${r.id}`} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:border-teal-200 hover:bg-teal-50/40 transition-all">
              ...
              <div className="mt-6 p-4 rounded-2xl bg-teal-50 border border-teal-100 flex flex-wrap items-center justify-between gap-3">
                <p className="text-sm font-bold text-teal-800">تريد المزيد من الملخصات والملفات؟</p>
                <Link to={`/library?subject=${subject.id}&grade=${gradeId}`} className="bg-teal-600 text-white text-sm font-extrabold px-5 py-2.5 rounded-xl hover:bg-teal-700 transition-colors">تصفح مكتبة يُسر</Link>
              </div>
            </section>
          )}

          {tab === 'bank' && (
            <section className="bg-gradient-to-br from-teal-600 to-teal-700 text-white rounded-3xl p-9">
              <h2 className="text-2xl font-extrabold mb-2">📝 بنك أسئلة {subject.name}</h2>
              <p className="text-teal-100 leading-7 mb-6 max-w-2xl">آلاف الأسئلة مصنفة حسب الوحدة ومستوى الصعوبة، مع تصحيح فوري وشرح للإجابات. درّب نفسك على أسئلة الاختبارات الحقيقية.</p>
          ...
          {tab === 'exams' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to={`/exams?subject=${subject.id}&grade=${gradeId}`} className="text-teal-600 font-bold text-sm">جميع الاختبارات ←</Link>
              </div>
              {exams.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {exams.slice(0, 4).map((e) => (
                    e.locked ? (
                      <Link key={e.id} to="/pricing" className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:border-teal-200 hover:bg-teal-50/40 transition-all">
                      ...
                      <Link key={e.id} to={`/exams/${e.id}/take`} className="flex items-center gap-4 p-4 rounded-2xl border border-slate-100 hover:border-teal-200 hover:bg-teal-50/40 transition-all">
                        <div className="w-12 h-12 rounded-xl bg-teal-100 text-teal-700 flex items-center justify-center font-black shrink-0">📝</div>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-slate-800 leading-6 line-clamp-1">{e.title}</p>
                          <p className="text-xs text-slate-400">{e.exam_type} • {e.question_count} سؤال • {e.duration_minutes} دقيقة</p>
                        </div>
          ...
          {tab === 'live' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              <div className="flex items-center justify-between mb-6">
                ...
                <Link to={liveLink} className="text-teal-600 font-bold text-sm">جدول الحصص ←</Link>
              </div>
              ...
              <Link to={liveLink} className="inline-block mt-5 bg-teal-600 text-white font-extrabold px-7 py-3.5 rounded-2xl hover:bg-teal-700 transition-colors">شاهد الحصص المباشرة</Link>
            </section>
          )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {teachers.map((t) => (
                    <div key={t} className="flex items-center gap-4 p-5 rounded-2xl border border-slate-100">
                      ...
                      <Link to={`/lessons?teacher=${encodeURIComponent(t)}`} className="text-teal-600 font-bold text-sm whitespace-nowrap">استكشف محتواه ←</Link>
                    </div>
                  ))}
                </div>
              )}
          ...
          {tab === 'discussions' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              ...
              {user ? (
                <form onSubmit={sendMsg} className="mb-6 p-4 rounded-2xl bg-teal-50 border border-teal-100">
                  <textarea value={msg} onChange={(e) => setMsg(e.target.value)} rows={2} placeholder={`اكتب سؤالك أو مشاركتك عن ${subject.name}...`} className="w-full px-4 py-3 rounded-xl border border-slate-200 bg-white focus:outline-none focus:ring-2 focus:ring-teal-400 text-sm" />
                  <button type="submit" disabled={sending || !msg.trim()} className="mt-3 bg-teal-600 text-white font-extrabold px-6 py-2.5 rounded-xl text-sm hover:bg-teal-700 transition-colors disabled:opacity-50">
                  ...
                  {discussions.map((d) => (
                    <div key={d.id} className="p-4 rounded-2xl border border-slate-100 bg-slate-50/40">
                      <div className="flex items-center gap-2 mb-1.5">
                        <span className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-black shrink-0 ${d.user_role === 'teacher' ? 'bg-emerald-500' : d.user_role === 'admin' ? 'bg-red-500' : 'bg-teal-500'}`}>{d.user_name?.charAt(0) ?? '؟'}</span>
          ...
          {tab === 'share' && (
            <section className="bg-white rounded-3xl border border-slate-100 shadow-sm p-7">
              ...
              {user ? (
                <div className="flex flex-wrap gap-3">
                  <Link to="/dashboard" className="bg-teal-600 text-white font-extrabold px-7 py-3.5 rounded-2xl hover:bg-teal-700 transition-colors">متابعة تقدمي</Link>
                ...
                <Link to="/register" className="inline-block bg-teal-600 text-white font-extrabold px-7 py-3.5 rounded-2xl hover:bg-teal-700 transition-colors">أنشئ حسابك مجاناً</Link>
              )}
            </section>
          )}
        </div>
